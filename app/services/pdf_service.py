...
# Register Unicode fonts for Naira symbol support
try:
    pdfmetrics.registerFont(TTFont('DejaVuSans', '/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf'))
    pdfmetrics.registerFont(TTFont('DejaVuSans-Bold', '/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf'))
    logger.info("✅ Unicode fonts registered for Naira symbol support")
...
class PDFGenerator:
    ...
    def _load_stamp(self):
        ...
        try:
            ...
            if stamp_path.exists():
                ...
                logger.info("✅ Stamp loaded successfully")
                return True
            ...
            possible_paths = [
                ...
                Path("/opt/render/project/src/static/images/stamp.png"),
                Path("/app/static/images/stamp.png"),
            ]
    ...
    def _load_logo(self):
        ...
        try:
            ...
            possible_paths = [
                ...
                Path("/opt/render/project/src/static/images/logo.png"),
    ...
    def _create_header_footer(self, canvas_obj, doc, title="", is_terms=False, total_pages=999):
        ...
        footer_text = "This document is electronically generated by Marshal Core of Nigeria. For official use only."
        canvas_obj.drawCentredString(page_width/2, 0.72*inch, footer_text)
        
        # STAMP on pages 2+ (not first page) EXCEPT LAST - bottom RIGHT corner
        # First page has inline stamp in content, last page has inline stamp with commandant
        current_page = canvas_obj._pageNumber
        # Only show footer stamp on page 2+ (not page 1, not last page)
        if self.stamp_bytes and current_page > 1 and current_page < total_pages:
            try:
                self.stamp_image.seek(0)
                stamp_width = 1.0 * inch
                stamp_height = 0.65 * inch
                stamp_x = page_width - doc.rightMargin - stamp_width - 0.15*inch  # Bottom RIGHT
                stamp_y = 1.2 * inch  # Above footer line, non-conflicting position
    ...
    def generate_officer_terms_conditions(self, user_data: Dict[str, Any], user_id: str) -> str:
        ...
        try:
            ...
            total_pages = 3  # Terms & Conditions
            def on_first_page(canvas_obj, doc_obj):
                self._create_header_footer(canvas_obj, doc_obj, "Terms & Conditions", is_terms=True, total_pages=total_pages)
    ...
    def generate_officer_application_form(self, user_data: Dict[str, Any], user_id: str) -> str:
        ...
        try:
            ...
            # Organization signature with inline stamp - stamp at bottom RIGHT on LAST page
            ...
            # Build PDF with watermark
            total_pages = 2  # Application Form
            def on_first_page(canvas_obj, doc_obj):
                self._create_header_footer(canvas_obj, doc_obj, "Application Form", total_pages=total_pages)
                self._add_background_watermark(canvas_obj, doc_obj)
